...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    The skeleton's ``.trellis/config.yaml`` ships ``dispatch_mode:
    sub-agent`` as the rendered default (the spawn_agent PreToolUse hook
    injects file-backed task context, so child agents do not rely on
    parent-session state). This resolver's own fallback for a missing or
    invalid key stays ``inline``: a config without the knob may predate
    the sub-agent support files, and main-session-direct editing is the
    safe degradation.

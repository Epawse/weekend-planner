...
0. **Verify implementation is allowed**:
   ```bash
   python3 ./.trellis/scripts/task.py current --source
   python3 ./.trellis/scripts/task.py validate <current-task>
   ```
   Continue only when the current task exists, `task.json.status` is `in_progress`, and validation does not report `Planning gate: 未通过`. If the task is still `planning`, stop and return to `trellis-brainstorm` / Phase 1 instead of editing code.

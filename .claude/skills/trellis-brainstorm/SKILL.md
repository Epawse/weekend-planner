...
## When to Recommend grill-me (one-shot, never insist)

grill-me stays optional. After the PRD converges, recommend it **once** — a single sentence naming the concrete weak spots — when at least one signal fires:

- An acceptance criterion stayed vague after two attempts, or the user answered "你看着办" on a scope question.
- The task is P0/P1 with cross-package or cross-repo blast radius.
- Requirements reversed direction at least once during this brainstorm.
- You can see an unstated tension between two requirements (fast vs simple, compatible vs clean).

Template: "PRD 里 X、Y 两处还比较松，要不要跑一遍 `trellis-grill-me` 压一压（一轮、几分钟）？不跑就按当前理解继续。" If the user declines, record nothing and never re-raise it in this task.

...
Resolve the active task path. First, look at the dispatch prompt's first line; if it is `Active task: <path>`, use that path. Otherwise run `python3 ./.trellis/scripts/task.py current --source` → active task path. If neither yields a task path, ask the user where to write output; do NOT guess.

...
# Bootstrap notice for Codex while the session has no active task. It is useful
# for sessions where SessionStart hooks have not yet been trusted or enabled.
...
def _backlog_armed_block(root: Path) -> str | None:
    """Return a compact codex-only BACKLOG armed block, if any.

    The parser lives in .trellis/scripts/common/backlog_context.py. Keep this
    hook as a consumer so BACKLOG markdown parsing stays centralized.
    """
    scripts_dir = root / ".trellis" / "scripts"
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
    try:
        ...
        armed = get_backlog_armed_line(root)
    except Exception:
        return None
    if not armed:
        return None
    return f"<backlog-armed>\n{armed}\n</backlog-armed>"
...
def _codex_mode_banner(config: dict) -> str:
    ...
    `inline` when missing or invalid because Codex child agents should not
    rely on parent-session state. Sub-agent mode is available when the
    dispatch prompt carries `Active task: <path>` and the agent loads task
    artifacts from files. The banner makes the active mode explicit to
...
def resolve_breadcrumb_key(
    ...
    Codex defaults to ``inline`` because child agents should not rely on
    parent-session state. Users can opt into ``codex.dispatch_mode:
    sub-agent`` in ``.trellis/config.yaml`` to use the parallel
    ``<status>-inline`` tag → ``<status>`` flip; the spawned agent must get
    an ``Active task: <path>`` dispatch line and load file-state context.
    Invalid or missing values fall back to inline.
...
def main() -> int:
    ...
    if platform == "codex":
        ...
        armed_block = _backlog_armed_block(root)
        if armed_block:
            parts.append(armed_block)

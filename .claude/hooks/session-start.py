...
""")
    ...
    try:  # M6 本地接线：注入 BACKLOG 武装条目（共享解析器在 .trellis/scripts/common）
        sys.path.insert(0, str(trellis_dir / "scripts"))
        from common.backlog_context import get_backlog_armed_line
        armed = get_backlog_armed_line(trellis_dir.parent)
        if armed:
            output.write(f"\n{armed}")
    except Exception:
        pass
    ...
    local_setup_hints = build_local_setup_hints(trellis_dir.parent)
    ...
    memory_block = build_memory_block(trellis_dir.parent)
    if memory_block:

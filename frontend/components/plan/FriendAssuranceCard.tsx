...
import { AlertTriangle, Camera, CheckCircle2, Info, MessageCircle, Sparkles, Users } from "lucide-react";
import type { EvidenceItem, FriendCheck, Plan } from "@/lib/types";

interface FriendAssuranceCardProps {
  plan: Plan;
}

function getCheckIcon(status: FriendCheck["status"]) {
  if (status === "pass") return <CheckCircle2 className="h-4 w-4 text-green-600" />;
  if (status === "fail") return <AlertTriangle className="h-4 w-4 text-red-600" />;
  return <Info className="h-4 w-4 text-amber-600" />;
}

function getCheckClass(status: FriendCheck["status"]): string {
...
function getFitLevel(score: number | null | undefined): string {
...
export function FriendAssuranceCard({ plan }: FriendAssuranceCardProps) {
  const profile = plan.friend_profile;
  const checks = plan.friend_checks ?? [];
  ...
  return (
    <section className="space-y-4 rounded-lg border border-sky-200 bg-sky-50/60 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <Users className="h-5 w-5 text-sky-600" />
            <h3 className="text-sm font-semibold text-zinc-900">朋友局适配检查</h3>
          </div>
          {profile && (
            <p className="mt-1 text-xs text-zinc-600">
              {profile.group_composition} · {profile.preferences.join("、")} · 晚餐{profile.dinner_window}
            </p>
          )}
        </div>
        <Badge className="border-sky-200 bg-white text-sky-700" variant="outline">
          朋友局适配：{getPlanFitLevel(plan)}
        </Badge>
      </div>

      {plan.friend_summary && (
        <p className="rounded-md bg-white px-3 py-2 text-xs text-zinc-700">{plan.friend_summary}</p>
      )}

      <div className="grid gap-2 sm:grid-cols-3">
        <Badge className="justify-center gap-1 border-sky-200 bg-white text-sky-700" variant="outline">

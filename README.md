...
| **LLM** | DeepSeek（默认，思考模式）· 通义千问 Qwen · OpenAI（可插拔，按已配置的 Key 自动选择） |
...
| 变量 | 说明 |
|---|---|
| `DEEPSEEK_API_KEY` | DeepSeek（默认 LLM） |
...
| `THINKING_ENABLED` / `THINKING_EFFORT` | DeepSeek 思考模式开关 / 强度 |
| `DEFAULT_LLM_PROVIDER` | 默认 LLM 提供方 |

...
| **LLM** | DeepSeek · 通义千问 Qwen · Google Gemini · OpenAI（可插拔；默认 Qwen，fallback 优先级 Qwen → DeepSeek → Gemini → OpenAI，仅使用已配置 Key 的 provider） |
...
> LLM fallback 优先级为 **Qwen → DeepSeek → Gemini → OpenAI**，仅使用已配置 Key 的 provider。
